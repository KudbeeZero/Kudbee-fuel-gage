...
// Phase 6: cryptographically verify a signature generated by a registered partner
// agent against its on-file public key. Returns the agentId when valid.
...
// Verify an AgentPass (agentId:issuedAt signed) against a supplied public key.
// Used for self-issued partner identities that are not in the static registry.
...
db.serialize(() => {
  ...
  // Phase 6: Governance ledger. Every partner "Verify" action is logged here as a
  // cryptographically signed governance action (type: 'GOVERNANCE_ACTION').
  db.run(`
  ...
  // Phase 6: community value score attribute, backfilled onto traces when verified.
  db.run(`ALTER TABLE telemetry_traces ADD COLUMN value_score REAL NOT NULL DEFAULT 0`, () => {
    /* column may already exist on older DBs — ignore the error */
  });
});
...
// Partner "Verify": a registered agent cryptographically signs a telemetry trace
// with its private key and submits the signature. The server verifies the
// signature against the on-file public key, then logs the action as a
// type: 'GOVERNANCE_ACTION' record and attributes the community value score.
app.post('/api/interceptor/verify', async (req, res) => {
  try {
    ...
    // Authenticate the agent pass (proves liveness / identity session). When the
    // partner supplies its own public key (self-issued identity), verify the pass
    // signature against it; otherwise fall back to the on-file registry.
    ...
    // Cryptographically verify the trace signature. Prefer a public key supplied by
    // the agent (Phase 6 Partner Portal: agents self-issue identity), falling back
    // to the on-file registry public key for known agents.
    ...
    // Attribute the value score onto the trace when present.
    if (score > 0) {
      await runQuery(
...
// Governance Feed: the ledger of verified (signed-off) traces, newest first.
...
// Community Value Score: aggregate of all verified trace value scores plus the
// governance participation count. This is the headline metric shown on the portal.
