...
export function useTelemetryLogger(onNewLogTriggered?: () => void) {
  ...
  const handleInjectTrace = useCallback(
    async (selectedModel: string, tokenCount: number, predictedOutputTokens: number) => {
      setIsLogging(true);
      const modelMap: Record<string, { provider: string; model_name: string }> = {
        ...
        'Mercury 2': { provider: 'Inception', model_name: 'mercury-2' },
      };
      const mapped = modelMap[selectedModel] || {
        provider: 'Anthropic',
        model_name: selectedModel.toLowerCase().replace(/\s+/g, '-'),
      };
