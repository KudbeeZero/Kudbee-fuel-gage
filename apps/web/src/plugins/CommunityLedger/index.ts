...
const CommunityLedgerView = () => {
  const [state, setState] = React.useState<any>({ decisions: [], source: 'loading', error: null });
  React.useEffect(() => {
    let cancelled = false;
    async function load() {
      try {
        ...
        if (!cancelled) setState({ decisions: d.decisions ?? d.ledger ?? [], source: 'ledger', error: null });
      } catch (e) {
  ...
  return React.createElement(
    ...
    React.createElement('h3', { style: { margin: '0 0 12px', fontSize: 14, color: '#94a3b8' } }, `Decision Ledger · ${list.length}`),
    state.error
      ? React.createElement('p', { style: { color: '#f87171', fontSize: 12 } }, `error: ${state.error}`)
      : React.createElement(
          'div',
          { style: { display: 'flex', flexDirection: 'column', gap: 6 } },
          list.slice(-8).reverse().map((d: any) =>
            React.createElement('div', { key: d.id ?? d.mission ?? Math.random(), style: { display: 'flex', justifyContent: 'space-between', padding: '6px 8px', background: '#0f172a', borderRadius: 6, fontSize: 12 } },
              React.createElement('span', { style: { color: '#e2e8f0', fontFamily: 'ui-monospace,monospace', fontSize: 11 } }, d.id ?? ''),
              React.createElement('span', { style: { color: '#94a3b8', maxWidth: '60%', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' } }, String(d.problem ?? d.mission ?? '').slice(0, 40)))
          )
      ),
