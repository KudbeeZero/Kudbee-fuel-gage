...
export function LearningCenter({ records: external, profiles: externalProfiles, onSearch }: LearningCenterProps) {
  ...
  const records: LearningRecord[] = external ?? [
    { id: 'r1', observation: 'Deployment encountered 2 failures', rootCause: 'Missing rollback configuration', recommendation: 'Add rollback config before deploy', confidence: 0.8, category: 'deployment', severity: 'high', scope: 'local', createdAt: new Date(Date.now() - 3600000).toISOString(), source: 'pipeline', context: { agents: ['FORGE'], services: ['AWS EC2'], files: [] }, validationResults: [] },
