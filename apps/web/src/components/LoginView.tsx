...
export function LoginView({ onAuthenticate }: { onAuthenticate: () => void }) {
  ...
  const handleLogin = async () => {
    if (isBooting || !passkey.trim()) return;
    ...
    try {
      const response = await fetch(apiUrl('/api/auth/session'), {
        ...
        headers: {
          ...
          'X-Agent-Pass': passkey.trim(),
        },
        credentials: 'include',
      });

      if (!response.ok) {
        let message = 'ACCESS DENIED. INVALID AGENT PASS.';
        try {
          const payload = (await response.json()) as { message?: string };
          if (payload.message) message = payload.message.toUpperCase();
        } catch {
          // Keep the stable login error when the server does not return JSON.
        }
      ...
      if (!_mountedRef.current) return;
      setError('GATEWAY UNAVAILABLE. SESSION WAS NOT ISSUED.');
      setPasskey('');
    }
  };
        ...
        {!isBooting ? (
          ...
          >
            ...
            <div className="space-y-6">
              <div className="space-y-2">
                <label className="font-mono text-xs text-slate-400 uppercase tracking-wider block">Agent Pass</label>
                <input
                  type="password"
                  value={passkey}
              ...
              <button
                ...
                disabled={isBooting || !passkey.trim()}
          ...
          >
            <div className="font-mono text-emerald-400 text-sm space-y-2">
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }}>[SYSTEM] Server-issued session established...</motion.div>
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }}>[SYSTEM] HttpOnly session cookie secured...</motion.div>
