...
export function PlaygroundView({ currency, onNewLogTriggered }: PlaygroundViewProps) {
  ...
  const [selectedModel, setSelectedModel] = usePersistentState<string>(
    'kudbee.playground.selectedModel',
    'Claude 3.5 Sonnet'
  );
  ...
  const [lastPolicy, setLastPolicy] = useState<{
    ...
    results: PlaygroundResult['policyResults'];
  } | null>(null);
  ...
  const displayOutput =
    isTyping || !lastResult ? streamedContent : streamedContent || lastResult.content;
  ...
  const modelSpecs = useMemo(
    () => ({
      Anthropic: { costIn: 3.0, costOut: 15.0, speed: 75, quality: 9.8, color: '#f97316' },
      DeepSeek: { costIn: 0.55, costOut: 2.19, speed: 120, quality: 9.5, color: '#3b82f6' },
      Google: { costIn: 1.25, costOut: 5.0, speed: 100, quality: 8.8, color: '#a855f7' },
      OpenAI: { costIn: 5.0, costOut: 15.0, speed: 80, quality: 9.2, color: '#ec4899' },
      Ternary: { costIn: 0.8, costOut: 3.0, speed: 110, quality: 9.0, color: '#10b981' },
    }),
    []
  );
  ...
  const rates = useMemo(
    () => ({
      Anthropic: { in: 3.0, out: 15.0, name: 'Claude 3.5 Sonnet', platform: 'Anthropic API' },
      DeepSeek: { in: 0.55, out: 2.19, name: 'DeepSeek-R1', platform: 'DeepSeek Cloud' },
      Google: { in: 1.25, out: 5.0, name: 'Gemini 1.5 Pro', platform: 'Google AI Studio' },
      OpenAI: { in: 5.0, out: 15.0, name: 'GPT-4o', platform: 'OpenAI Developer Platform' },
      Ternary: { in: 0.8, out: 3.0, name: 'Ternary Bonsai 27B', platform: 'Ternary Network' },
    }),
    []
  );

  const calculateCost = useCallback(
    (prov: keyof typeof rates) => {
  ...
  const selectedModelId =
    (comparisons.find((c) => c.name === selectedModel)?.id as keyof typeof rates) || 'Anthropic';
  ...
  return (
    <div className="space-y-6" id="playground-view-container">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div
          className="lg:col-span-7 bg-slate-900/60 border border-slate-800 rounded-xl p-6 flex flex-col justify-between relative overflow-hidden"
          id="playground-calculator"
        >
          ...
          <div>
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="font-display font-semibold text-slate-200 text-lg">
                Telemetry Cost Calculator
              </h2>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2">
                ...
                <div className="flex items-end">
                  <button
                    onClick={() =>
                      handleInjectTrace(selectedModel, tokenCount, predictedOutputTokens)
                    }
                    disabled={isLogging}
              ...
              {policyClasses && (
                ...
                >
                  ...
                  <span className="opacity-80">
                    {lastPolicy?.results.filter((r) => r.status !== 'PASS').length || 0} rule(s)
                    flagged
                  </span>
                </div>
              )}
        ...
        <div
          className="lg:col-span-5 bg-slate-950/80 border border-slate-800 rounded-xl flex flex-col relative overflow-hidden"
          id="playground-response-terminal"
        >
          ...
          <div className="flex items-center justify-between px-4 py-3 border-b border-slate-800/60 bg-slate-900/40">
            ...
            <div className="flex items-center gap-1.5">
              <span
                className={`w-2 h-2 rounded-full ${isRunning ? 'bg-amber-400 animate-pulse' : isTyping ? 'bg-cyan-400 animate-pulse' : lastResult ? 'bg-emerald-400' : 'bg-slate-700'}`}
              />
              <span className="text-[9px] font-mono uppercase tracking-widest text-slate-500">
                {isRunning
                  ...
                  : isTyping
                    ? 'STREAMING'
                    : lastResult
                      ? `${lastResult.route} · ${lastResult.latencyMs}ms`
                      : 'IDLE'}
              </span>
              <button
                id="open-rag-drawer"
          ...
          <div className="flex-1 min-h-[280px] max-h-[420px] overflow-y-auto p-4 font-mono text-xs leading-relaxed">
            {displayOutput ? (
              ...
              <div className="h-full flex flex-col items-center justify-center gap-3 text-slate-500">
                ...
                <p className="font-mono text-[10px] text-slate-700">POST /v1/chat/completions</p>
              </div>
            ) : (
              <div className="h-full flex flex-col items-center justify-center gap-3 text-slate-600">
                ...
                <div className="text-center">
                  ...
                  <p className="text-slate-700 font-mono text-[10px] mt-1.5">
                    Run <span className="text-emerald-500/70">RUN STREAM</span> or{' '}
                    <span className="text-emerald-500/70">DIRECT-INJECT</span> to populate the
                    channel.
                  </p>
                </div>
          ...
          <div className="px-4 py-2 border-t border-slate-800/60 bg-slate-900/20 flex items-center justify-between text-[10px] font-mono text-slate-600">
            <span>
              MODEL: <span className="text-slate-400">{selectedModel}</span>
            </span>
            {lastResult ? (
              <span className="flex items-center gap-3">
                <span className="text-emerald-400">
                  {lastResult.tokensIn}+{lastResult.tokensOut}t
                </span>
                <span className="text-amber-400">
                  {getFormattedCost(lastResult.cost, currency, 4)}
                </span>
      ...
      <div
        ...
        id="playground-thresholds"
      >
        <div className="absolute top-0 left-0 w-full h-[1px] bg-gradient-to-r from-transparent via-emerald-500/50 to-transparent"></div>

        <div className="flex items-center gap-2 mb-6">
          <Sliders className="w-5 h-5 text-emerald-400" />
          <h2 className="font-display font-semibold text-slate-200 text-md">
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-center text-xs font-mono text-slate-400">
              ...
              <span className="text-emerald-400 font-bold">
                {getFormattedCost(singleCap, currency, 2)}
              </span>
            </div>
            <input
              id="hard-cap-slider"
            ...
            <div className="flex justify-between text-[9px] font-mono text-slate-600">
              ...
              <span>{getFormattedCost(1.0, currency, 2)}</span>
              <span>{getFormattedCost(2.0, currency, 2)}</span>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between p-4 bg-slate-950 rounded-xl border border-slate-800/60">
            <div>
              <span className="block text-xs font-semibold text-slate-200">
                Hourly Run-Rate Warning
              </span>
              <span className="block text-[10px] text-slate-500 mt-1">
                Warn if projected run-rate exceeds quota bounds.
              </span>
            </div>
            <button
            ...
            >
              <div
                className={`w-4 h-4 bg-slate-950 rounded-full shadow transition-all duration-300 ${
            ...
            <>
              ...
              <div>
                ...
                <span className="text-[11px] text-amber-500/80 block mt-0.5">
                  The current telemetry calculation for {selectedModel} (
                  {getFormattedCost(selectedCost, currency, 6)}) violates your budget hard-cap limit
                  of {getFormattedCost(singleCap, currency, 2)}. Modify prompt context or route to
                  optimal provider.
                </span>
              </div>
            </>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          <div className="rounded-lg border border-slate-800 bg-slate-950/40 p-3">
            ...
            <div className="mt-1 font-mono text-lg font-bold text-slate-300">
              {vectorStatus.recentDocs.length}
            </div>
          </div>
          <div className="rounded-lg border border-slate-800 bg-slate-950/40 p-3">
            <div className="text-[10px] font-mono uppercase tracking-widest text-slate-500">
            ...
            <div className="mt-1 font-mono text-sm font-bold text-slate-300">
              {vectorStatus.lastSyncAt
                ? new Date(vectorStatus.lastSyncAt).toLocaleTimeString()
                : '—'}
            </div>
          </div>
        </div>
