  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      if ((res.status === 429 || res.status === 503) && attempt < retries) {
        ...
        console.warn(
          `[apiClient] ${res.status} on ${url}, retrying in ${delay}ms (attempt ${attempt + 1}/${retries})`
        );
        await sleep(delay);
        continue;
      }
      ...
      if (
        ...
        if (attempt < retries) {
          ...
          console.warn(
            `[apiClient] Network error on ${url}, retrying in ${delay}ms (attempt ${attempt + 1}/${retries})`
          );
          await sleep(delay);
          continue;
        }
        throw new NetworkError(`Network unreachable: ${url}`, {
          cause: err,
        });
      }
      throw err;
